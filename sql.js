...
const mostSeasonWins = (country, season) => {
  if (['Summer', 'Winter', 'Fall', 'Spring'].includes(season)) {
...
*/
const countryBest = (thing, country) => {
  if (['year', 'discipline', 'sport', 'event'].includes(thing)) {
    return `SELECT ${thing}, COUNT(*) AS count FROM GoldMedal
    WHERE country = '${country}'
    GROUP BY 1
    ORDER BY 2 DESC
    LIMIT 1;`;
...
const bestYear = country => {
  return countryBest('year', country);
};
...
const bestDiscipline = country => {
  return countryBest('discipline', country);
};
...
const bestSport = country => {
  return countryBest('sport', country);
};
...
const bestEvent = country => {
  return countryBest('event', country);
};

/*
Returns a SQL query string that will find the number of male medalists.
*/
const numOfMedalistsThatAre = (gender, country) => {
...
const numberMenMedalists = country => {
  return numOfMedalistsThatAre('Men', country);
};
...
const numberWomenMedalists = country => {
  return numOfMedalistsThatAre('Women', country);
};
...
*/
const orderingString = (field, sortAscending) => {
  let orderingString = "";
  if (field) {
    if (sortAscending) {
...
const orderedMedals = (country, field, sortAscending) => {
  return `SELECT * FROM GoldMedal 
    WHERE country = '${country}'
    ${orderingString(field, sortAscending)}`;
};
